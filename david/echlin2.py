...
def process_links(links):
    ...
    pairsNum = len(node1)
    node_num=len(set(node1+node2))
    nodeList = range(node_num)
    ## node frequencies
    freqCount = np.zeros(node_num)
    for k in nodeList:
    ...
    if node_num <= 3:
        print("Sample %s has too few(%i) nodes" %(file,node_num))
    else:
        #print 'correlations'
        ## distance properties
        ...
        localOptFrac = np.divide(localOptNum, node_num, dtype = float)          
